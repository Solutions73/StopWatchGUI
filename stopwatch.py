...
def create_window():
  sg.theme('black')
  layout = [
      # [sg.Push(), sg.Image('cross.pg', pad= 0, enable_events = True, key = '-CLOSE-')],
      [sg.VPush()],
      ...
      # [sg.Column([[sg.Text('row1')], [sg.Text('row2')]])],
      [sg.Column([[]], key= '-LAPS-')],
      [sg.VPush()]
      ]
  return sg.Window(
...
while True:
    ...
    if event == '-LAP-':
      #  window.extend_layout(window, [[sg.Text('test')]])  # window.extend_layout(container, element)
